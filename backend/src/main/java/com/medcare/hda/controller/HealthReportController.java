...
import com.medcare.hda.service.HealthReportGenerationService;
...
public class HealthReportController {
    ...
    private final HealthReportGenerationService generationService;
    ...
    @Operation(summary = "结构化报告详情（兼容旧报告）")
    @GetMapping("/{id}/detail")
    public Result<HealthReportDetailVO> detail(@PathVariable Long id) {
        HealthReport entity = service.getById(id);
        checkOwner(entity);
        return Result.success(generationService.detail(entity));
    }
    ...
    @Operation(summary = "一键生成结构化健康报告")
    @PostMapping("/generate")
    public Result<HealthReportDetailVO> generate(@RequestBody(required = false) HealthReportGenerateRequest request) {
        Long userId = SecurityUtil.getUserId();
        return Result.success("报告已生成", generationService.generate(userId, request));
    }
