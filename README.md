...
注意：不要直接执行未指定 `--path` 的 `chroma run`，否则数据会写入当前目录（例如从用户主目录启动时会落到 C 盘）。请优先使用下面的项目脚本，它会将数据固定保存到 `E:\SECP\data\rag\chroma`；若使用全局 Chroma，也应显式执行 `chroma run --path E:\SECP\data\rag\chroma --host 127.0.0.1 --port 8000`。
...
- `DELETE /api/consult/sessions/{sessionId}`：删除当前用户自己的健康助手会话及其消息、问诊状态和运行记录；不会删除已沉淀的长期记忆。
