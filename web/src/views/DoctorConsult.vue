      ...
      <template v-if="active">
        <header class="chat-head">
          <div class="doc">
            ...
            <div>
              ...
              <div class="doc-sub">{{ active.doctor?.hospital }} · {{ active.doctor?.department }}</div>
            </div>
          </div>
          <el-tag type="success">实时咨询</el-tag>
        </header>
        ...
        <footer class="input-row">
          <el-upload :show-file-list="false" :http-request="doUpload">
            <el-button :loading="uploading">附件</el-button>
          </el-upload>
          <el-input v-model="text" size="large" placeholder="请输入想咨询医生的问题…" @keyup.enter="send" />
          <el-button type="primary" size="large" @click="send">
...
<style scoped>
.chat-card { height: calc(100vh - 152px); }
.chat-card :deep(.el-card__body) { height: 100%; display: grid; grid-template-columns: 320px 1fr; padding: 0; }
.sessions { border-right: 1px solid var(--hda-line); overflow-y: auto; }
