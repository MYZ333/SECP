<template>
  <div class="admin-overview hda-enter">
    <OverviewHero
      ...
      title="系统管理"
      description="统一维护平台用户、积分商品、药品库、医生专家和智能体知识库，快速进入常用后台管理任务。"
      cta-label="进入用户管理"
    ...
    <section class="preview-grid">
      ...
      <article class="preview-panel" style="--index: 2">
        <div class="panel-head"><h3>药品库</h3><el-button link type="primary" @click="go('/admin/medicine')">管理</el-button></div>
        <el-skeleton v-if="loading" :rows="4" animated />
        <template v-else>
          <div v-for="medicine in medicines" :key="medicine.id" class="line-row">
            <span>{{ medicine.name }}</span>
            <el-tag size="small" :type="medicine.status ? 'success' : 'info'" effect="plain">{{ medicine.status ? '启用' : '停用' }}</el-tag>
          </div>
          <el-empty v-if="!medicines.length" description="暂无药品数据" :image-size="70" />
        </template>
      </article>
      ...
      <article class="preview-panel" style="--index: 4">
...
import { ArrowRight, Box, Document, FirstAidKit, Goods, User } from '@element-plus/icons-vue'
import { adminPageDoctors, adminPageKnowledge, adminPageMedicines, adminPageProducts, adminPageUsers } from '@/api'
import OverviewHero from '@/components/OverviewHero.vue'

const router = useRouter()
const totals = ref({ user: 0, product: 0, medicine: 0, doctor: 0, knowledge: 0 })
const users = ref([])
const products = ref([])
const medicines = ref([])
const doctors = ref([])
...
const modules = [
  ...
  { title: '药品库', desc: '维护医生用药建议可选药品与用法提醒。', path: '/admin/medicine', icon: FirstAidKit },
...
const summaryItems = computed(() => [
  ...
  { label: '药品总数', value: totals.value.medicine },
...
async function load() {
  try {
    const [userRes, productRes, medicineRes, doctorRes, knowledgeRes] = await Promise.allSettled([
      adminPageUsers({ pageNum: 1, pageSize: 4 }),
      adminPageProducts({ pageNum: 1, pageSize: 4 }),
      adminPageMedicines({ pageNum: 1, pageSize: 4 }),
    ...
    if (medicineRes.status === 'fulfilled') {
      totals.value.medicine = medicineRes.value.data.total || 0
      medicines.value = medicineRes.value.data.records || []
    }
...
.summary-strip { display: grid; grid-template-columns: repeat(5, 1fr); gap: 1px; overflow: hidden; border: 1px solid var(--hda-line); background: var(--hda-line); }
...
.module-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(245px, 1fr)); gap: 16px; }
...
.preview-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(245px, 1fr)); gap: 16px; }
